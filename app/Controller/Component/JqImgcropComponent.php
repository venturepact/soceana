...
class JqImgcropComponent extends Component {
    ...
    function uploadImage($uploadedInfo, $uploadTo, $imageName){
        ...
        $tempFileName = basename($uploadedInfo["name"]);       
        $file_ext = substr($tempFileName, strrpos($tempFileName, ".") + 1);
...
$ext = strtolower(substr(basename($image), strrpos(basename($image), ".") + 1));
    ...
    function cropImage($thumb_width, $x1, $y1, $x2, $y2, $w, $h, $thumbLocation, $imageLocation){
        $scale = $thumb_width/$w;       
        $thumbnailNameArray = explode('img_',$thumbLocation);       
        $newThumbnailLocation = $thumbnailNameArray[0] .'thumb_'. $thumbnailNameArray[1];
        $image = array();
        $image['User']['image'] = 'img_'.$thumbnailNameArray[1];
        $image['User']['thumb_image'] = 'thumb_'.$thumbnailNameArray[1];
        $cropped = $this->resizeThumbnailImage(WWW_ROOT.str_replace("/", DS,$newThumbnailLocation),WWW_ROOT.str_replace("/", DS,$imageLocation),$w,$h,$x1,$y1,$scale);
        return $image;
    }
